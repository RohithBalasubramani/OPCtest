...
class Command(BaseCommand):
    ...
    def run_from_argv(self, argv):
        ...
        loop.run_until_complete(self.handle_async(*argv[2:]))
    ...
    async def handle_async(self, *args, **kwargs):
        ...
        try:
            ...
            while True:  # Infinite loop
                ...
                # Prepare a list of async tasks
                tasks = []

                for tag, (model_name, field_name) in tag_model_mapping.items():
                    task = asyncio.create_task(self.process_tag(client, tag, model_name, field_name))
                    tasks.append(task)
    ...
    async def process_tag(self, client, tag, model_name, field_name):
        try:
            # Access the OPC UA node directly using its NodeId
            node = client.get_node(tag)
            value = await node.read_value()  # Use async method

            # Dynamically get the model class using apps.get_model
            try:
                model_class = await sync_to_async(apps.get_model)('opc_app', model_name)
            except LookupError:
            ...
            def get_model_fields():
                return [field.name for field in model_class._meta.get_fields() if not field.is_relation]

            model_fields = await sync_to_async(get_model_fields)()
            ...
            def get_model_meta_fields():
                return model_class._meta.fields

            fields = await sync_to_async(get_model_meta_fields)()
            for field in fields:
