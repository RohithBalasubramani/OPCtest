...
class Command(BaseCommand):
    ...
    def fetch_data(self, client):
        table_names = {}
        model_instances = {}
        for value in list(tag_model_mapping.values()):
            if value[0] not in table_names:
                table_names[value[0]] = {'timestamp': timezone.now()}

        node_ids = list(tag_model_mapping.keys())
        
        for id in node_ids:
            node = client.get_node(id)
            reading = client.get_values([node])
            model_name, field_name = tag_model_mapping[id]
            table_names[model_name][field_name] = reading[0]
        
        for key in list(table_names.keys()):
            model_class = self.get_model_class(key)
            model_data = table_names[key]
            for key in list(model_data.keys()):
                if not self.is_field_in_model(key, model_class):
                    del model_data[key]
